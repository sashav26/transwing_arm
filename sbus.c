...
static volatile uint16_t ch1val = 0;

// SBUS Frame Definitions
#define SBUS_START_BYTE 0x0F
#define SBUS_END_BYTE   0x00 // Replace with actual end byte if different
#define SBUS_FRAME_SIZE 25
#define SBUS_BAUDRATE   100000 // Replace with actual SBUS baud rate
...
bool get_sbus_frame(uint8_t *frame_out, size_t frame_len) {
    ...
    size_t available = (rx_head + RX_BUFFER_SIZE - rx_tail) % RX_BUFFER_SIZE;
    if (available < frame_len) {
        return false; // Not enough data
    }

    // Check if the first byte is the start byte
    if (rx_buffer[rx_tail] != SBUS_START_BYTE) {
        return false; // Start byte mismatch
    }
...
// 6) RESYNC FUNCTION
// --------------------------
// Searches the buffer for the next start byte and updates rx_tail accordingly
bool resync_buffer() {
    while (rx_tail != rx_head) {
        ...
        // Move tail forward to skip invalid byte
        rx_tail = (rx_tail + 1) % RX_BUFFER_SIZE;
    }
    return false; // No start byte found
}

// --------------------------
// 7) MAIN PROCESS LOOP WITH RESYNC
// --------------------------
void process_sbus_data() {
    while (true) {
        ...
        if (get_sbus_frame(frame, SBUS_FRAME_SIZE)) {
            // We have a full SBUS frame
            if (validate_sbus_frame(frame)) {
                printf("== SBUS FRAME RECEIVED ==\n");
                for (int ch = 0; ch < 16; ch++) {
                    uint16_t val = sbus_get_channel(frame, ch);
                    printf("Channel %d: %u\n", ch + 1, val);
                    if (ch == 0) { // Assuming ch1val corresponds to Channel 1
                        ch1val = val;
                    }
                }
            } else {
                printf("[ERROR] Invalid SBUS frame. Attempting to resync...\n");
                // Attempt to resynchronize by searching for the next start byte
                if (!resync_buffer()) {
                    // No start byte found, wait for more data
                    break;
                }
            }
        } else {
            // Not enough data, attempt to resync if possible
            if (!resync_buffer()) {
                // No start byte found, wait for more data
                break;
            }
        ...
        // Optionally, add a small delay to prevent CPU hogging
        // sleep_ms(1);
    }
}